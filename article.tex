...
Then, the angles $\theta$  are $ \theta = \{0, 45, 90, 135, 180, 225, 270, 315\} $. For each spacial bin $ i,j = \{1,2,3,4\} $, corresponding to the indexes of each block $B_{i,j}$,  the orientations are accumulated in a  $3$-dimensional histogram $h$ through the following equation: %In each step, the histogram is computed by multiplying the length of the gradient vector by a Wang trilinear interpolation between the gradient angle and the bin, in order to give importance to the module of the gradient. 
...
\begin{figure}[H]
\centering
\subfigure[Plot of the signal, a keypoint and the corresponding patch.]{\includegraphics[width=.49\linewidth]{DiagramaConkeypoint.png}\label{patch}}
\subfigure[Orientations on each patch.]{\includegraphics[width=.49\linewidth]{histogramsbines.png}\label{histogrambines}}
...
\begin{itemize}
\item For a Reactive Synchronous speller, like the P300 Speller, we need to identify the selected letter 1 out of 36 (like the matrix).
\item Rows are 1-6 and columns are 7-12
\item The matrix is randomly highlighted, rows and columns, making a permutation 1-12.
\item So, each 12 stimulations there are one row and one column that matches the selected letter by the user and that it should generate the P300 waveform.
\item Thus, we need to identify the segment that generated the p300 response.
\item Descriptors are obtained from images of signal plots according to previously described algorithm.
\item Descriptors thus belong to two classes: hits vs no-hits
\item During training phase, descriptors from labeled segments are extracted.  These are the templates and they are grouped in a template set $ T $.
\item On decoding phase, a new query image is generated from the signal plot of a segment of single-channel EEG (in the same way as for training phase).  From this image a new \textit{query} descriptor is extracted.
\item so we need to identify the row, and the column.  There are 6 rows and 6 columns for a given letter.
\item the letter that the person was focusing on is i,j, corresponding to one out of the 6 rows, and one out of the 6 columns.
\item how we can do this ?  Well we generate 12 images, 6 for rows, and 6 for columns
\item we pick the first 6 rows, and get one descriptor for each.
\item we have 6 descriptors and we build 6 classes.
\item we compute \ref{eq:classification} and we pick the class that minimize it.
\item we thus have now the row.
\item repeat for column.
\item we can now identify the selected letter. 
\end{itemize}
...
While doing classification based on local features, which encode partial information, one problem that frequently arises is how to ensemble the individual classification of each local characteristic to the whole image. %that were used to derive those descriptors.
