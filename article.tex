...
\item Obtain the descriptor from the image of the signal plot according to the method described in Section~\ref{Feature}, $ d^r_1, \dots  d^r_6 $ for rows and  $ d^c_7, \dots  d^c_{12} $ for columns.
\item Compute  
...
\noindent for rows, and  repeat for the 6 columns:
...
Each $q$ that belongs to the $k$ nearest neighbors  $NN_T(d^c_u)$ is used to calculate the distances to the rows descriptors and the columns descriptors, and by minimizing the aforementioned equations, the letter of the matrix can be determined from the intersection of the row $ \hat{r} $ and column $ \hat{c} $.
