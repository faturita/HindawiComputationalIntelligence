...
\item For $ k= {1-6} $: Obtain the descriptor from the image of the signal plot according to the method described in Section~\ref{SIFT}, $ d^r_1, \dots,  d^r_6 $ for rows and  $ d^c_1, \dots,  d^c_6 $. 
\item Compute  
...
\label{eq:multiclassification}
\end{equation}
where $q$ is a possible descriptor, $NN_T(d)$ is the set of descriptors nearest to $d$, finally $\hat{r}$ and $\hat{c}$ are the optimal row and column.
