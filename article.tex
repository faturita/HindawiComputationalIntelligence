...
In the article~\citep{Ramele2016},  the authors introduce a method for classification of rhythmic EEG events like Visual Occipital Alpha Waves  and Motor Imagery rolandic central $\mu$ rhythms using the histogram of gradient orientations of signal plots.  Inspired in that work, we proposed a novel application of the developed method to classify and describe transient events, particularly the P300 Event Related Potential.  The proposed approach is based on the morphological analysis of the shape of the EEG signal, but using histogram descriptors. The method is built by mimicking what traditionally electroencephalographers have been performing for almost a century: visually inspecting raw signal plots, as it is described in~\citep{Hartman2005}.

Other works have explored the idea of describing the waveform of a signal. In~\citep{Alvarado-Gonzalez2016,Yamaguchi2009} an approach based on Slope Chain Code is presented, and also the work of Bondt-Pompe Permutation Entropy~\citep{Berger2017}.
...
\item  \label{paso2} Repeat item 1 $k_a$ times, obtaining the single trial segments $S_1(t,c),\dots,S_{k_a}(t,c)$, where the variables $t$ and $c$ correspond to time and channel, respectively. The parameter $k_a$ is the number of repetitions and it is an input parameter of the algorithm.
...
\item \label{paso4}Plot the signal $x(t,c)$, according Section~\ref{Plot}. 
...
\begin{itemize}
\item PODRIA ACA MOSTRAR ADEMAS PLOTS DE SINGLE TRIAL PARA QUE VEAN QUE SON UN QUILOMBO.
...
\item La pregunta:  ¿ Es posible utilizando solamente la información de los plots, aún utilizando preprocesamiento y una mejora en el SNR basado en el esquema tradicional de generación del patrón de p300, detectarlo con alto grado de éxito ?  
...
The method benefical aspect is that its single channel approach and also the most important part is that the method allows close collaboration because templates can be selected by visually inspecting them.
