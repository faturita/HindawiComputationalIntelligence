...
\begin{enumerate}
\item Highlight randomly the rows and columns from the matrix.  There are one row and one column that matches the selected letter by the user.
\item For $ k= {1-6} $: Obtain the descriptor from the image of the signal plot according to the method described in Section~\ref{Feature}, $ d^r_1, \dots  d^r_6 $ for rows and  $ d^c_1, \dots  d^c_6 $ 
\item For $ k= {7-12} $: Compute  
...
\noindent This procedure is repeated for the 6 columns.
