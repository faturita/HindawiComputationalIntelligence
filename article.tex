...
On the generated image, a keypoint $\mathbf{kp}$ is placed on a pixel $(x_kp, y_kp)$ over the image plot. In Figure  \ref{fig:sampledescriptor}.  Around the keypoint, a local image patch is constructed by dividing it in 16 blocks of size $ 3 \cdot s $ where $ s is the size of the local patch $, arranged in a 4x4 grid and centered on $ \mathbf{kp} = (x_kp,y_kp)$, the patch center.
...
\begin{figure}[H]
\centering
\includegraphics[width=5cm]{descriptorpatchsample.png}
\includegraphics[width=6cm]{histogramsbines.png}
