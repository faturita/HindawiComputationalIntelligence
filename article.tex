...
The preprocessing stage consists of the enhancement of the SNR of the P300 pattern above the level of basal EEG. 
...
The aim is to identify the selected letter from the matrix. Previously, during training phase, descriptors from \textit{hit} labeled segments are extracted.  These descriptors are the P300 templates which are grouped in a template set called $ T $.  Segments corresponding to rows are labeled 1-6, whereas those corresponding to columns are labeled 7-12.  The process has the following steps:
...
\item Repeat item 1 $k_a$ times, obtaining the single trial segments $S_1(t,c),\dots,S_{k_a}(t,c)$, where the variables $t$ and $c$ correspond to time and chanel, respectively. The parameter $k_a$ is an input of the algorithm, and corresponds to the number of repetitions.
\item Compute  
\begin{equation}
...
We replicated the same experiment on healthy subjects using a wireless digital EEG device (g.Nautilus, g.Tec, Austria).  The experimental conditions were the same as those used for the previous dataset, as detailed in section~\ref{ALSDataset}.

Participants were recruited voluntarily and the experiment were conducted anonymously in accordance with the declaration of Helsinki published by the World Health Organization.  No monetary compensation was handed out and all participants agreed and signed a written informed consent.  All healthy subjects had normal or corrected-to normal vision and no history of neurological disorders. The experiment was performed with 20 subjects, 7 males, 4 females, average age 22.5 years, standard deviation  1,8 years, range 21–26 years.

Gel-based active electrodes (g.LADYbird, g.Tec, Austria) were used on the same locations Fz, Cz, P3, Pz, P4, PO7, PO8 and Oz.  Reference was set to the right ear lobe and ground was preset as the AFz position.   Sampling frequency is slightly different, and was set to 250 Hz, which is the closest possible to the one used with the other dataset.
...
Parameters are selected according to the experimental protocol. First of all, the patch size in pixels is $S_P = 3s.4 \times 3s.4$. The P300 event can have a span of $400$ ms and its amplitude can reach $ 10 \mu V $.  Hence it is necessary to utilize a size patch $S_P$ that could capture an entire transient event. With this purpose in consideration, the $s$ value election is essential.
...
\noindent where $\Delta  \mu V $ corresponds to the amplitude in microvolts that can be covered by the height of the patch, $ Fs $ is the sample frequency of the EEG signal (downsampled to 16 Hz) and $ \lambda $ is the length in seconds covered by the patch. The geometric structure of the patch forces a squared configuration, then we discerned that by using $ s =s_x =s_y = 3 $ and $ \gamma = 4 $  the local patch and the descriptor can identify events of 9 $ \mu V $ of amplitude, with a span of $ \lambda = 0.56$ seconds.  This also provides a resolution of $ 1 $ Pixel representing $  \frac{1}{4} \mu V $ in the vertical direction and $\frac{1}{Fs \cdot \gamma}$ seconds in the horizontal direction. Finally, descriptors  $\mathbf{kp}$  are located at $ x =  0.55 Fs.\gamma = 35 $ and $ y = z(c) $ (Eq.~\ref{eq:zerolevel}).   In this way the whole transient event should be captured. 
...
Results are shown in Table \ref{tab:results} where the percentage of correctly spelled letters is calculated while performing an offline BCI Simulation.  From the 7 words for each subject, the first 3 were used as training, and the remaining 4 for testing.  The best performing channel is informed as well.  

DRAFT: Traditionally P300 can be seen in this channel and this other channel.  However, Wolpaw showed that the response may also arise in occipital channels as well.  The authors of the original paper concluded that Cz was the best performing channel.  However, in our approach 


It is of particular interest that using this method, the best performing channel was not always Cz, and instead occipital channels PO8 and PO7 showed higher performances \citep{Huggins2016,Jure2016}.
