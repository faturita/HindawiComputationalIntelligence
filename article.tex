...
Parameters are selected according to the experimental protocol. It is define in Section~\ref{Feature}, the patch size is $S_P = 3s.4 \times 3s.4$ .
As the P300 event latency and amplitude vary greatly between subjects, it is necessary to utilize a size patch $S_P$ that be able to capture an entire transient event. With this purpose the $s$ value election is essential.
...
We proposed the equations~\ref{eq:mapping2} and~\ref{eq:mapping1} for compute the scale value in horizontal and vertical directions, respectively. 
\begin{equation}
...
\noindent where $\Delta  \mu V $ corresponds to the amplitude in microvolts that can be covered by the height of the patch, $ Fs $ is the sample frequency of the EEG signal (downsampled to 16 Hz) and $ \lambda $ is the length in seconds covered by the patch. In this way the whole event is capture. But we need a squared patch, then we discovered that by using $ s =s_x =s_y = 3 $ and $ \gamma = 4 $  the local patch and the descriptor can identify events of 9 $ \mu V $ of amplitude, span of $ \lambda = 0.56$ seconds.  The resolution of $ 1 $ Pixel representing $  \frac{1}{4} \mu V $ in the vertical direction and $\frac{1}{Fs}\gamma$ in horizontal direction. Finally, descriptors  $\mathbf{kp}$  are located at $ x =  0.55 Fs.\gamma = 35 $ and $ y = z(c) $ (Eq.~\ref{eq:zerolevel}). 
