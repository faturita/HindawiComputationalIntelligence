...
%\end{itemize}
%%JG No entiendo nada
%We find that only by using the visual aspect of the P300 signal plot,  for some subjects it was not possible to find templates that allow  to reach a higher level classification.  
...
% Escribir conclusiones buenas sobre todo los mismos puntos que se tocan en la discusion
% Si bien 

% El tema del estudio lo mismo que hace el medico pero de manera automatico.

% Letter.  Esto esta aca.

%The SIFT method is invariant to the scale, so in order to allow our method to be also invariant to the amplitude of the signal we had to modify the geometry of the patch to remove the squared constrain.
